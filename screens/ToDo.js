...
export default function ToDo({ navigation }) {
  ...
  let loadToDoList = async () => {
    const q = query(collection(db, "todos"), where("todo_user", "==", auth.currentUser.uid));
  ...
  let addToDo = async (todoName, todoFreq) => {
    let toDoToSave = {
      todo_name: todoName,
      todo_freq: todoFreq,
      todo_check: false,
      todo_user: auth.currentUser.uid
    };
  ...
  return (
    <ImageBackground style={AppStyles.imageContainer} source={background}>
      ...
      <SafeAreaView style={AppStyles.backgroundCover} >
        <Text style={[AppStyles.lightText, AppStyles.header, AppStyles.topMargin]}>ToDo</Text>
      </SafeAreaView>
      {auth.currentUser.emailVerified ? [showContent(), showToDoList()] : showSendVerificationEmail()}
    </ImageBackground>
  );
}